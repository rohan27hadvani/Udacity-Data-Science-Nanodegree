...
def index():
    ...
    #visual 2
    top_category_mean = df[df.columns[4:]].sum().sort_values(ascending=False).head(3)
    top_category_names = list(top_category_mean.index)
    #visual 3
    category_count = df[df.columns[4:]].sum()/len(df)
